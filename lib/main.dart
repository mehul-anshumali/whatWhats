...
class _MyHomePageState extends State<MyHomePage>
  ...
  void _downloadStatus() async {
    try {
      Directory? appDir = await getExternalStorageDirectory();
      if (appDir != null) {
        Directory statusDir =
            Directory("${appDir.path}/WhatsApp/Media/.Statuses");
        List<FileSystemEntity> statusList = statusDir.listSync();

        for (var status in statusList) {
          if (status is File) {
            // Copy the status file to the app directory
            String fileName = status.path.split('/').last;
            File newFile = File("${appDir.path}/$fileName");
            await status.copy(newFile.path);
